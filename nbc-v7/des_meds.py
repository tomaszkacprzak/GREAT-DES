...
def write_meds(file_name, obj_list, clobber=True):
    ...
    cols = []
    cols.append( pyfits.Column(name='number', format='i4', array=range(n_obj) ) )
    ...
    cols.append( pyfits.Column(name='file_id', format='%di4' % MAX_NCUTOUTS , array=numpy.array([[1]]*n_obj)) )
    cols.append( pyfits.Column(name='start_row', format='%di4' % MAX_NCUTOUTS,
                                                    array=numpy.array(cat['start_row'])) )
    cols.append( pyfits.Column(name='orig_row', format='%df8' % MAX_NCUTOUTS, array=numpy.array([[1]]*n_obj) ) )
    cols.append( pyfits.Column(name='orig_col', format='%df8' % MAX_NCUTOUTS, array=numpy.array([[1]]*n_obj) ) )
    cols.append( pyfits.Column(name='orig_start_row', format='%di4' % MAX_NCUTOUTS,
                                                    array=numpy.array([[1]]*n_obj) ) )
    cols.append( pyfits.Column(name='orig_start_col', format='%di4' % MAX_NCUTOUTS,
                                                    array=numpy.array([[1]]*n_obj) ) )
...
def BuildMEDS(file_name, config, nproc=1, logger=None, file_num=0, image_num=0, obj_num=0):
    ...
    # TK hack to make scaling and noise variance same as in DES
    if 'des_scale_pixels' in config:
        for i in range(len(main_images)):

            main_array = main_images[i].array
            main_array*=config['des_scale_pixels']
            weight_array = weight_images[i].array
            weight_array*= 1./config['des_scale_pixels']**2
